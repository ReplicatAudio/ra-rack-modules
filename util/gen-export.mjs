...
// ---- TEST HOOK ----
const srcPath0 = process.env.TEST_MOD;
if (srcPath0) {
  const info0 = parseModule(srcPath0);
  console.log("HP=", info0.HP, "HW=", info0.HW);
  for (const c of info0.components) {
    if (c.role==="input"||c.role==="output") console.log(c.role, String(c.enum).slice(0,22).padEnd(24), "x="+c.x, "y="+c.y);
  }
  process.exit(0);
}
