...
const SW = 0.6;   // stroke width for outlines
...
svg += `  <rect x="${dm.x.toFixed(2)}" y="${dm.y.toFixed(2)}" width="${dm.w.toFixed(2)}" height="${dm.h.toFixed(2)}" rx="1" fill="#0a0f0a" stroke="#1a3a1a" stroke-width="${SW + 0.2}"/>\n`;

// Module name
svg += `  <text x="${cx.toFixed(2)}" y="${(ru2mm(10)).toFixed(2)}" fill="#555" font-family="sans-serif" font-size="2.8" text-anchor="middle" font-weight="bold">ENDLESS</text>\n`;
...
for (const el of elements) {
  ...
  if (el.kind === 'in' || el.kind === 'out') {
    ...
    const r = ru2mm(dim.r);
    svg += `  <circle cx="${mx.toFixed(2)}" cy="${my.toFixed(2)}" r="${r.toFixed(2)}" fill="#111" stroke="${c}" stroke-width="${SW + 0.1}" opacity="0.7"/>\n`;
    svg += `  <circle cx="${mx.toFixed(2)}" cy="${my.toFixed(2)}" r="${(r * 0.35).toFixed(2)}" fill="${c}" opacity="0.5"/>\n`;
    // Label below
    const ly = my + r + 2.0;
    svg += `  <text x="${mx.toFixed(2)}" y="${ly.toFixed(2)}" fill="${c}" font-family="sans-serif" font-size="2.0" text-anchor="middle" opacity="0.7">${el.label}</text>\n`;

  } else if (el.kind === 'switch') {
    // CKSS toggle: small rectangle + lever line
    const hw = ru2mm(dim.w);
    const hh = ru2mm(dim.h);
    svg += `  <rect x="${(mx - hw).toFixed(2)}" y="${(my - hh).toFixed(2)}" width="${(hw * 2).toFixed(2)}" height="${(hh * 2).toFixed(2)}" rx="1" fill="none" stroke="${c}" stroke-width="${SW - 0.1}" opacity="0.5"/>\n`;
    // Lever
    svg += `  <line x1="${mx.toFixed(2)}" y1="${(my + hh * 0.5).toFixed(2)}" x2="${mx.toFixed(2)}" y2="${(my - hh * 0.5).toFixed(2)}" stroke="${c}" stroke-width="${SW + 0.2}" opacity="0.6"/>\n`;
    // Label below
    const ly = my + hh + 2.0;
    svg += `  <text x="${mx.toFixed(2)}" y="${ly.toFixed(2)}" fill="${c}" font-family="sans-serif" font-size="2.0" text-anchor="middle" opacity="0.7">${el.label}</text>\n`;
    ...
    const r = ru2mm(dim.r);
    svg += `  <circle cx="${mx.toFixed(2)}" cy="${my.toFixed(2)}" r="${r.toFixed(2)}" fill="#222" stroke="${c}" stroke-width="${SW}" opacity="0.6"/>\n`;
    ...
    const r = ru2mm(dim.r);
    svg += `  <circle cx="${mx.toFixed(2)}" cy="${my.toFixed(2)}" r="${r.toFixed(2)}" fill="#1a1a1a" stroke="${c}" stroke-width="${SW + 0.1}" opacity="0.6"/>\n`;
...
for (const ly of lineY) {
  svg += `  <line x1="2" y1="${ly.toFixed(2)}" x2="${(W - 2).toFixed(2)}" y2="${ly.toFixed(2)}" stroke="#2a2a2a" stroke-width="${SW - 0.2}"/>\n`;
}
...
for (const sec of sections) {
  ...
  svg += `  <text x="2.5" y="${sy.toFixed(2)}" fill="#444" font-family="sans-serif" font-size="1.8" font-weight="bold">${sec.text}</text>\n`;
}

// Track labels for outputs
svg += `  <text x="${ru2mm(15).toFixed(2)}" y="${ru2mm(295).toFixed(2)}" fill="#555" font-family="sans-serif" font-size="1.8" text-anchor="middle">A</text>\n`;
svg += `  <text x="${ru2mm(15).toFixed(2)}" y="${ru2mm(335).toFixed(2)}" fill="#555" font-family="sans-serif" font-size="1.8" text-anchor="middle">B</text>\n`;
